        ...
        if (response.functionCalls && response.functionCalls.length > 0) {
            for (const funcCall of response.functionCalls) {
                if (funcCall.name === 'findAlternativeRoute') {
                    // FIX: Explicitly cast function call arguments to their expected types.
                    const currentLocation = funcCall.args.currentLocation as string;
                    const destination = funcCall.args.destination as string;
                    ...
                    // FIX: Explicitly cast function call argument from 'unknown' to 'string'. This resolves the TypeScript error.
                    const message = funcCall.args.message as string;
